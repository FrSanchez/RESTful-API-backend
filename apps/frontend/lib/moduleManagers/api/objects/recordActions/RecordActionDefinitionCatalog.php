...
use Api\Actions\ActionDefinitionCatalog;
use Api\Actions\FileSystemStaticActionDefinitionCatalog;
use Api\Actions\StaticActionDefinitionCatalog;
use Exception;
use sfContext;

class RecordActionDefinitionCatalog implements ActionDefinitionCatalog
{
	...
	/**
	 * Factory method for creating the {@see StaticActionDefinitionCatalog} for record actions.
	 * @return StaticActionDefinitionCatalog
	 */
	public static function createRecordActionStaticActionDefinitionCatalog(): StaticActionDefinitionCatalog
	{
	 ...
	 * @return false|RecordActionDefinition
	 */
	...
	private function initialize(int $version, int $accountId): void
	{
		...
		foreach ($objectNamesWithActions as $objectName) {
			...
			foreach ($staticActionNames as $actionName) {
				$staticActionDefinition = $this->staticActionDefinitionCatalog
					->findStaticActionDefinitionByObjectAndName($objectName, $actionName);

				if (!$accountSettingsManager->evaluateFeatureFlagAccessRule(
					$staticActionDefinition->getRequiredFeatureFlags()
				)) {
				...
				$lowercaseObjectName = strtolower($objectName);
				$lowercaseActionName = strtolower($actionName);
				$actionDefinition = new RecordActionDefinition($staticActionDefinition, $accountId, $version);
