 ...
 */
abstract class AbstractProspectFilterByLastActivityProcedure implements \Api\Config\Objects\Prospect\Gen\ExportProcedures\ProspectFilterByLastActivityProcedureInterface
{
	/** @var int $accountId */
	...
	protected final function validate(array $arguments): void
	{
		$this->validateWithArgs(
			...
			$arguments[self::ARG_LAST_ACTIVITY_AFTER] ?? null,
			$arguments[self::ARG_LAST_ACTIVITY_BEFORE] ?? null
		);
	}
	...
	public final function generateDoctrineQuery(QueryContext $queryContext, array $selectedFields): Doctrine_Query
	{
		return $this->generateDoctrineQueryWithArgs(
			...
			$this->arguments[self::ARG_LAST_ACTIVITY_AFTER] ?? null,
			$this->arguments[self::ARG_LAST_ACTIVITY_BEFORE] ?? null
		);
	}
