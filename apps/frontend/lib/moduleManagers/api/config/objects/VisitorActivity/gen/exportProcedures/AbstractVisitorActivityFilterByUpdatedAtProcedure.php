 ...
 */
abstract class AbstractVisitorActivityFilterByUpdatedAtProcedure implements \Api\Config\Objects\VisitorActivity\Gen\ExportProcedures\VisitorActivityFilterByUpdatedAtProcedureInterface
{
	/** @var int $accountId */
	 ...
	 * @throws Exception
	 */
	public function __construct(ProcedureDefinition $definition, int $version, int $accountId, array $arguments, bool $isInternalRequest, string $object = 'VisitorActivity')
	{
	...
	/**
	 * Gets the object definition for VisitorActivity.
	...
	protected final function validate(array $arguments): void
	{
		$this->validateWithArgs(
			$arguments[self::ARG_PROSPECT_ONLY] ?? null,
			$arguments[self::ARG_TYPE] ?? null,
	...
	public final function generateDoctrineQuery(QueryContext $queryContext, array $selectedFields): Doctrine_Query
	{
		return $this->generateDoctrineQueryWithArgs(
			$queryContext,
			$selectedFields,
			$this->arguments[self::ARG_PROSPECT_ONLY] ?? null,
			$this->arguments[self::ARG_TYPE] ?? null,
